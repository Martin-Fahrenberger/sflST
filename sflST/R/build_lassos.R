...
build_lassos <- function(counts_matrix,coords_table,name,ncores=8,include_diag=FALSE,output_folder,gamma=1){
    require(genlasso)
    require(parallel)
    source('./Chunks/fused_lasso_complete_fixed_gamma.R')
    source('./Chunks/build_expanded_neighbours.R')
    ...
    mclapply(1:length(list_of_sets),function(x) fused_lasso_complete_fixed_gamma(list_of_sets[[x]],counts_matrix,coords_table,paste(name,x,sep='_'),include_diag,output_folder,gamma=gamma),mc.cores = ncores)
}
